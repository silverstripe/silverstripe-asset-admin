// This file was generated by silverstripe/tx-translator from client/lang/src/it.json.
// See https://github.com/silverstripe/silverstripe-tx-translator for details
if (typeof(ss) === 'undefined' || typeof(ss.i18n) === 'undefined') {
  if (typeof(console) !== 'undefined') { // eslint-disable-line no-console
    console.error('Class ss.i18n not defined');  // eslint-disable-line no-console
  }
} else {
  ss.i18n.addDictionary('it', {
    "AssetAdmin.ADD_FILES": "Aggiungi da file",
    "AssetAdmin.ADD_FOLDER_BUTTON": "Aggiungi cartella",
    "AssetAdmin.ADD_NEW_FOLDER_BUTTON": "Aggiungi nuova cartella",
    "AssetAdmin.BACK": "Indietro",
    "AssetAdmin.BACK_DESCRIPTION": "Vai al livello superiore",
    "AssetAdmin.BACK_TO_HISTORY": "Torna alla cronologia",
    "AssetAdmin.BROWSE": "Sfoglia",
    "AssetAdmin.BULK_ACTIONS_ARCHIVE": "Archivio",
    "AssetAdmin.BULK_ACTIONS_ARCHIVE_FAIL_02": "%s cartelle/file sono stati archiviati con successo ma non è stato possibile archiviare %s file.",
    "AssetAdmin.BULK_ACTIONS_ARCHIVE_FOLDERS_CONFIRM": "Sei sicuro di voler archiviare queste cartelle?",
    "AssetAdmin.BULK_ACTIONS_ARCHIVE_FOLDER_CONFIRM": "Sei sicuro di voler archiviare questa cartella?",
    "AssetAdmin.BULK_ACTIONS_ARCHIVE_ITEMS_CONFIRM": "Stai per archiviare %s file che può essere stato usato nei contenuti. Controllare attentamente come è stato usato prima di procedere.",
    "AssetAdmin.BULK_ACTIONS_ARCHIVE_SUCCESS_02": "%s cartelle/file sono state archiviate con successo.",
    "AssetAdmin.BULK_ACTIONS_ARCHIVE_WARNING": "Assicurarsi che i file siano stati rimossi dai contenuti prima di archiviarli, altrimenti verranno visualizzati come link non funzionanti.",
    "AssetAdmin.BULK_ACTIONS_CLEAR_SELECTION": "Annulla selezione",
    "AssetAdmin.BULK_ACTIONS_CONFIRM": "Sei sicuro di voler %s questi file?",
    "AssetAdmin.BULK_ACTIONS_DELETE": "Elimina",
    "AssetAdmin.BULK_ACTIONS_DELETE_CONFIRM": "Sei sicuro di voler eliminare questi file?",
    "AssetAdmin.BULK_ACTIONS_DELETE_FAIL": "%s cartelle/file sono stati archiviati con successo ma non è stato possibile archiviare %s file.",
    "AssetAdmin.BULK_ACTIONS_DELETE_FAIL_02": "%s cartelle/file eliminati correttamente, but non è stato possibile eliminare%s file.",
    "AssetAdmin.BULK_ACTIONS_DELETE_FOLDER": "Queste cartelle contengono file attualmente in uso, devi spostare o eliminare il suo contenuto prima di poter eliminare una cartella.",
    "AssetAdmin.BULK_ACTIONS_DELETE_FOLDERS_CONFIRM": "Sei sicuro di voler eliminare queste cartelle?",
    "AssetAdmin.BULK_ACTIONS_DELETE_FOLDER_CONFIRM": "Sei sicuro di voler eliminare questa cartella?",
    "AssetAdmin.BULK_ACTIONS_DELETE_ITEMS_CONFIRM": "Stai per eliminare %s file che può essere stato usato nei contenuti. Controllare attentamente come è stato usato prima di procedere.",
    "AssetAdmin.BULK_ACTIONS_DELETE_MULTI_CONFIRM": "Ci sono %s file attualmente in uso, sei sicuro di voler eliminare questi file?",
    "AssetAdmin.BULK_ACTIONS_DELETE_SINGLE_CONFIRM": "Questo file è attualmente usato in %s posti, se sicuro di volerlo eliminare?",
    "AssetAdmin.BULK_ACTIONS_DELETE_SUCCESS": "%s cartelle/file sono state archiviate con successo.",
    "AssetAdmin.BULK_ACTIONS_DELETE_SUCCESS_02": "%s cartelle/file eliminati correttamente.",
    "AssetAdmin.BULK_ACTIONS_DELETE_WARNING": "Assicurarsi che i file siano stati rimossi dai contenuti prima di eliminarli, altrimenti verranno visualizzati come link non funzionanti.",
    "AssetAdmin.BULK_ACTIONS_PLACEHOLDER": "Scegliere un'azione...",
    "AssetAdmin.BULK_ACTIONS_SELECTED": "%s selezionati",
    "AssetAdmin.BULK_ACTIONS_SELECT_ALL": "Selezionare tutto",
    "AssetAdmin.CANCEL": "Annulla",
    "AssetAdmin.CATEGORY_ARCHIVE": "archivio",
    "AssetAdmin.CATEGORY_DOCUMENT": "documento",
    "AssetAdmin.CATEGORY_FOLDER": "cartella",
    "AssetAdmin.CATEGORY_IMAGE": "immagine",
    "AssetAdmin.CONFIRMDELETE": "Sei sicuro di voler eliminare questo record?",
    "AssetAdmin.CONFIRM_FILE_ARCHIVE": "Confermare archiviazione",
    "AssetAdmin.CONFIRM_FILE_DELETION": "Confermare eliminazione",
    "AssetAdmin.CREATED": "Caricato il",
    "AssetAdmin.CreateTitle": "Inserire nuovo materiale dal web",
    "AssetAdmin.DELETE": "Elimina",
    "AssetAdmin.DIM": "Dimensioni",
    "AssetAdmin.DROPZONE_CANCEL_UPLOAD": "Annulla caricamento",
    "AssetAdmin.DROPZONE_CANCEL_UPLOAD_CONFIRMATION": "Sei sicuro di voler annullare questo caricamento?",
    "AssetAdmin.DROPZONE_DEFAULT_MESSAGE": "Rilascia qui i file da caricare",
    "AssetAdmin.DROPZONE_FAILED_UPLOAD": "Caricamento file fallito",
    "AssetAdmin.DROPZONE_FALLBACK_MESSAGE": "Il tuo browser non supporta il caricamento file tramite trascinamento.",
    "AssetAdmin.DROPZONE_FALLBACK_TEXT": "Usare il modulo alternativo sottostante per caricare i fie alla vecchia maniera.",
    "AssetAdmin.DROPZONE_FILE_TOO_BIG": "File troppo grande. Dimensione massima: %sMiB.",
    "AssetAdmin.DROPZONE_INVALID_FILE_TYPE": "Non puoi caricare file di questo tipo.",
    "AssetAdmin.DROPZONE_MAX_FILES_EXCEEDED": "Non puoi più caricare file.",
    "AssetAdmin.DROPZONE_REMOVE_FILE": "Elimina file",
    "AssetAdmin.DROPZONE_RESPONSE_ERROR": "Il server ha risposto con errore.",
    "AssetAdmin.DROPZONE_SUCCESS_UPLOAD": "File caricato",
    "AssetAdmin.DROPZONE_UPLOAD": "Carica",
    "AssetAdmin.EDIT": "Modifica",
    "AssetAdmin.EMPTY": "Nessun file",
    "AssetAdmin.EditTitle": "Materiale dal web",
    "AssetAdmin.FILENAME": "Nome file",
    "AssetAdmin.FILES": "File",
    "AssetAdmin.FILE_GALLERY": "Galleria file",
    "AssetAdmin.FILE_MISSING": "Il file non può essere trovato",
    "AssetAdmin.FILE_TITLE": "Titolo: {title}",
    "AssetAdmin.FILTER_DATE_ASC": "più vecchio",
    "AssetAdmin.FILTER_DATE_DESC": "più nuovo",
    "AssetAdmin.FILTER_TITLE_ASC": "titolo a-z",
    "AssetAdmin.FILTER_TITLE_DESC": "titolo z-a",
    "AssetAdmin.IMAGE_EDITOR_CANCEL": "Annulla",
    "AssetAdmin.IMAGE_EDITOR_RESIZE_HEIGHT": "Altezza",
    "AssetAdmin.IMAGE_EDITOR_RESIZE_WIDTH": "Larghezza",
    "AssetAdmin.JOINLAST": "e",
    "AssetAdmin.LASTEDIT": "Ultima modifica",
    "AssetAdmin.LOADMORE": "Carica ancora",
    "AssetAdmin.NEXT": "Prossimo",
    "AssetAdmin.NOITEMSFOUND": "Nessun elemento trovato",
    "AssetAdmin.NOT_EDITABLE": "non modificabile",
    "AssetAdmin.NOT_SELECTABLE": "non selezionabile",
    "AssetAdmin.NUM_ITEMS_THIS_PAGE": "{numFolders} cartelle e {numFiles} file in questa pagina",
    "AssetAdmin.OR": "o",
    "AssetAdmin.PREVIOUS": "Precedente",
    "AssetAdmin.PROMPTFOLDERNAME": "Inserire il nome della cartella (o lasciare vuoto per annullare)",
    "AssetAdmin.REPlACE_FILE_SUCCESS": "Caricamento eseguito, il file sarà sostituito con Salva.",
    "AssetAdmin.SAVE": "Salva",
    "AssetAdmin.SEARCHCLEARRESULTS": "Annulla ricerca",
    "AssetAdmin.SEARCHRESULTS": "Risultati ricerca",
    "AssetAdmin.SEARCHRESULTSMESSAGE": "Risultati ricerca {parts}",
    "AssetAdmin.SEARCHRESULTSMESSAGECATEGORY": "catalogato come '{appCategory}'",
    "AssetAdmin.SEARCHRESULTSMESSAGEEDITEDBETWEEN": "ultima modifica tra '{lastEditedFrom}' e '{lastEditedTo}'",
    "AssetAdmin.SEARCHRESULTSMESSAGEEDITEDFROM": "ultima modifica dal '{lastEditedFrom}'",
    "AssetAdmin.SEARCHRESULTSMESSAGEEDITEDTO": "ultima modifica prima del '{lastEditedTo}'",
    "AssetAdmin.SEARCHRESULTSMESSAGEKEYWORDS": "con i termini '{name}'",
    "AssetAdmin.SEARCHRESULTSMESSAGELIMIT": "ristretta alla cartella '{folder}'",
    "AssetAdmin.SELECT": "Scegliere",
    "AssetAdmin.SELECTABLE": "selezionabile",
    "AssetAdmin.SIZE": "Dimensione",
    "AssetAdmin.TITLE": "Titolo",
    "AssetAdmin.TYPE": "Tipo file",
    "AssetAdmin.UPLOADING_INSTRUCTION": "in upload, premere invio per annullare",
    "AssetAdmin.UPLOAD_FAILED_INSTRUCTION": "upload fallito, premere invio per rimuovere",
    "File.DRAFT": "Bozza",
    "File.MODIFIED": "Modificato",
    "File.REMOVE": "Rimuovi",
    "File.VIEW": "Vista"
});
}